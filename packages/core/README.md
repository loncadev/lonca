...
| Primitive                | Purpose                                                                                                                                                    |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Money`, `Currency`      | ISO 4217 currency codes; integer minor-unit money representation                                                                                           |
| `CursorPage`, `paginate` | Cursor-based pagination + async iterator helper                                                                                                            |
| `LoncaError` hierarchy   | Structured errors (`AuthError`, `RateLimitError`, `ValidationError`, `NotFoundError`, `ServerError`, `NetworkError`, `TimeoutError`) with `retryable` flag |
...
- **Money is an integer in minor units** — `{ amount: 12550, currency: 'TRY' }` means 125.50 TRY. No floats, no surprises.
